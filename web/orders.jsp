...
<html>
    ...
    <body>
        ...
        <c:if test="${sessionScope.ACCOUNTLOGGED !=null && sessionScope.ACCOUNTLOGGED.role.roleID == 2}">
            <div class="row">
                   ...
                   <div class="sidebar-widget">
                        <div class="sidebar-widget mb-40">
                            <div class="sidebar-widget mb-35">
                                <h4 class="sidebar-widget-title">Trạng thái đơn hàng</h4>
                                <div class="sidebar-widget-list">
                                    <ul>
                                        <li>
                                            <div class="sidebar-widget-list-left">
                                                <label><input type="checkbox" name="status" value="0"
                                                              ${orderFilter.getSelectStatus()[0] ? 'checked' : ''}>Đang xử lý</label>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="sidebar-widget-list-left">
                                                <label><input type="checkbox" name="status" value="1"
                                                              ${orderFilter.getSelectStatus()[1] ? 'checked' : ''}>Chấp nhận đơn hàng</label>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="sidebar-widget-list-left">
                                                <label><input type="checkbox" name="status" value="2"
                                                              ${orderFilter.getSelectStatus()[2] ? 'checked' : ''}>Từ chối đơn hàng</label>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="sidebar-widget-list-left">
                                                <label><input type="checkbox" name="status" value="3"
                                                              ${orderFilter.getSelectStatus()[3] ? 'checked' : ''}>Đang giao hàng</label>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="sidebar-widget-list-left">
                                                <label><input type="checkbox" name="status" value="4"
                                                              ${orderFilter.getSelectStatus()[4] ? 'checked' : ''}>Hoàn thành</label>
                                            </div>
                                        </li>
                                    </ul>
                                </div>
                ...
                <div class="col-lg-7">
                    <div class="sidebar-widget">
                        <div class="sidebar-widget mb-40">
                            <h4 class="sidebar-widget-title">Tìm kiếm đơn hàng</h4>
                            <div class="sidebar-search">
                                <form class="sidebar-search-form" id="search-order">
                                    <input type="text" placeholder="Nhập mã hoặc tên đơn hàng" name="search"
                    ...
                    <div class="shop-topbar-wrapper">
                        <div class="shop-topbar-left">
                            <div class="view-mode nav tooltip-style-2">
                                <a class="active" href="<%=request.getContextPath()%>/orders"
                                   title="Thiết lập lại mặc định"><i class="fa fa-refresh"></i></a>
                            </div>
                                <c:if test="${orderFilter.getTotalOrder() > 0}">
                                    <p>Hiển thị ${(orderFilter.getCurrentPage()-1)*orderFilter.getOrderPerPage() +1 }
                                ...
                                <c:if test="${orderFilter.getTotalOrder() <= 0}">
                                    <div class="alert alert-danger text-center" style="margin: 0px 20px 0px 20px">
                        ...
                        <div class="product-sorting-wrapper">
                            <div class="product-show shorting-style">
                                ...
                                <select name="sortBy" id="sortBy">
                                    <option value="HistoryTime" ${(orderFilter.getSortBy() eq "HistoryTime") ? "selected" : "" }> Lịch sử hoạt động
                                    </option>
                                    <option value="OrderDate" ${(orderFilter.getSortBy() eq "OrderDate") ? "selected" : "" }> Ngày đặt đơn hàng
                                    </option>
                                    <option value="Order_ID" ${(orderFilter.getSortBy() eq "Order_ID") ? "selected" : "" }> Mã đơn hàng
                                    </option>
                                    <option value="Fullname" ${(orderFilter.getSortBy() eq "Fullname") ? "selected" : "" }> Tên khách hàng
                                    </option>
                                    <option value="TotalTransaction" ${(orderFilter.getSortBy() eq "TotalTransaction") ? "selected" : "" }> Tổng giá đơn hàng
                                    </option>
                                </select>
                            </div>
                        </div>
                        <button id="sortSide" value="${orderFilter.isIsAsc()}">
                            <c:if test="${orderFilter.isIsAsc()}"><i class="fa fa-sort-asc"></i></c:if>
                            <c:if test="${!orderFilter.isIsAsc()}"><i class="fa fa-sort-desc"></i></c:if>
                        </button>
                    </div>
                </div>
            </div>
            <c:if test="${orders.size()>0}">
                <div id="orders">
                                ...
                                <tbody>
                                    <c:forEach items="${orders}" var="o">
                                        <tr>
                                            ...
                                            <td>
                                                <button onclick="getOrderDetail(${o.getId()})" class="check-btn sqr-btn" id="detail-order-${o.getId()}">Hiện</button>
                                            </td>
                                            <td>
                                                <c:if test="${o.getStatusId() == 1}">
                                                    <button class="cancel-btn sqr-btn" onclick="cancelOrder(${o.getId()}, ${o.getSaleId()})">Từ chối</button>
                                                    <button class="cancel-btn sqr-btn" onclick="acceptOrder(${o.getId()}, ${o.getSaleId()})">Chấp thuận</button>
                                                </c:if>
                                                    ...
                                                    <div class="row">
                                                            ...
                                                            <div class="card">
                                                                ...
                                                                <div class="card-body card-block">
                                                                    <div class="form-group col-md-6" style="float: left">
                                                                        ...
                                                                        <input type="text" class="form-control" readonly="" value="${o.getPhone()}">
                                                                    </div>
                                                                    <div class="form-group col-md-6" style="float: right">
                                                                        <label class=" form-control-label">Email</label>
                                                                        <input type="text" class="form-control" readonly="" value="${o.getEmail()}">
                                                                    </div>
                                                                    <div class="form-group col-md-6" style="float: left">
                                                                        <label class=" form-control-label">Địa chỉ giao hàng</label>
                                                                        <textarea class="form-control" readonly="">${o.getAddress()}</textarea>
                                                                    </div>
                                                                    <div class="form-group col-md-6" style="float: right">
                                                                        <label class=" form-control-label">Ghi chú</label>
                                                                        <textarea class="form-control" readonly="">${o.getNotes()}</textarea>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="col-lg-12">
                                                            <div class="card">
                                                                ...
                                                                <div class="card-body card-block">
                                                                    <div class="myaccount-table table-responsive text-center" id="order-product-${o.getId()}">
                                                                        <table class="table table-bordered">
                                                                            <thead class="thead-light">
                                                                                <tr>
                                                                                    <th>Sản phẩm</th>
                    ...
                    <div class="pro-pagination-style text-center mt-10" id="pagination" style="padding-bottom: 50px">
                    </div>
                </div>
            </c:if>
        </c:if>
        ...
        <script>
            ...
            let sortSide = ${orderFilter.isIsAsc()};console.log(sortSide);
            let values = {};
            const loadPage = (page) => {
