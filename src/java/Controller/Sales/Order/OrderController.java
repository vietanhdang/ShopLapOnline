 ...
 */
package Controller.Sales.Order;
...
public class OrderController extends HttpServlet {
   private OrderFilter getOrderFilter(HttpServletRequest request) {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/sale/order/orders.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("acceptOrderId") != null) { // if sale accept order
            ...
            System.out.println("check response write   " + response.getWriter().toString());
        }
        if (request.getParameter("deliveryOrderId") != null) { // if sale confirm delivering order
            executeOrder(request, response, "deliveryOrderId", 4); // set order status to delivering
        }
        request.getRequestDispatcher("/sale/order/orders.jsp").forward(request, response);
    }
